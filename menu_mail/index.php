...
if ($_SESSION['scriptcase']['device_mobile'])
{
    ...
    //check prod
    if(empty($_SESSION['scriptcase']['menu_mail']['glo_nm_path_prod']))
    {
            /*check prod*/$_SESSION['scriptcase']['menu_mail']['glo_nm_path_prod'] = $str_path_apl_url . "_lib/prod";
    }
    //check tmp
    if(empty($_SESSION['scriptcase']['menu_mail']['glo_nm_path_imag_temp']))
    {
            /*check tmp*/$_SESSION['scriptcase']['menu_mail']['glo_nm_path_imag_temp'] = $str_path_apl_url . "_lib/tmp";
    }
    //end check publication with the prod

ob_start();

class menu_mail_class
{
  var $Db;
 ...
 function menu_mail_menu()
 {
    global $menu_mail_menuData, $nm_data_fixa;
     ...
     { 
         $_SESSION['scriptcase']['str_schema_all'] = $_POST["nmgp_schema"] . "/" . $_POST["nmgp_schema"];
           ...
           $nm_versao_sc  = "" ; 
           $_SESSION['scriptcase']['menu_mail']['contr_erro'] = 'off';
           $Campos_Mens_erro = "";
...
$path_libs      = $str_root . $_SESSION['scriptcase']['menu_mail']['glo_nm_path_prod'] . "/lib/php";
$path_third     = $str_root . $_SESSION['scriptcase']['menu_mail']['glo_nm_path_prod'] . "/third";
$path_adodb     = $str_root . $_SESSION['scriptcase']['menu_mail']['glo_nm_path_prod'] . "/third/adodb";
$path_apls      = $str_root . substr($path_link, 0, strrpos($path_link, '/'));
$path_img_old   = $str_root . $path_link . "menu_mail/img";
$this->path_css = $str_root . $path_link . "_lib/css/";
$_SESSION['scriptcase']['dir_temp'] = $str_root . $_SESSION['scriptcase']['menu_mail']['glo_nm_path_imag_temp'];
...
$this->sc_Include($path_libs . "/nm_ini_perfil.php", "F", "perfil_lib") ; 
 if(function_exists('set_php_timezone')) set_php_timezone('menu_mail');
...
if (!defined("SC_ERROR_HANDLER"))
{
    ...
    include_once(dirname(__FILE__) . "/menu_mail_erro.php");
}
include_once(dirname(__FILE__) . "/menu_mail_erro.class.php"); 
$this->Erro = new menu_mail_erro();
$str_path = substr($_SESSION['scriptcase']['menu_mail']['glo_nm_path_prod'], 0, strrpos($_SESSION['scriptcase']['menu_mail']['glo_nm_path_prod'], '/') + 1);
if (!is_file($str_root . $str_path . 'devel/class/xmlparser/nmXmlparserIniSys.class.php'))
{
    unset($_SESSION['scriptcase']['nm_sc_retorno']);
    unset($_SESSION['scriptcase']['menu_mail']['glo_nm_conexao']);
}

/* Definição dos Caminhos */
$menu_mail_menuData         = array();
$menu_mail_menuData['path'] = array();
$menu_mail_menuData['url']  = array();
$NM_dir_atual = getcwd();
if (empty($NM_dir_atual))
{
    $menu_mail_menuData['path']['sys'] = (isset($_SERVER['SCRIPT_FILENAME'])) ? $_SERVER['SCRIPT_FILENAME'] : $_SERVER['ORIG_PATH_TRANSLATED'];
    $menu_mail_menuData['path']['sys'] = str_replace("\\", '/', $str_path_sys);
    $menu_mail_menuData['path']['sys'] = str_replace('//', '/', $str_path_sys);
}
else
{
    $sc_nm_arquivo                                   = explode("/", $_SERVER['PHP_SELF']);
    $menu_mail_menuData['path']['sys'] = str_replace("\\", "/", str_replace("\\\\", "\\", getcwd())) . "/" . $sc_nm_arquivo[count($sc_nm_arquivo)-1];
}
$menu_mail_menuData['url']['web']   = $_SERVER['PHP_SELF'];
$menu_mail_menuData['url']['web']   = str_replace("\\", '/', $menu_mail_menuData['url']['web']);
$menu_mail_menuData['path']['root'] = substr($menu_mail_menuData['path']['sys'],  0, -1 * strlen($menu_mail_menuData['url']['web']));
$menu_mail_menuData['path']['app']  = substr($menu_mail_menuData['path']['sys'],  0, strrpos($menu_mail_menuData['path']['sys'],  '/'));
$menu_mail_menuData['path']['link'] = substr($menu_mail_menuData['path']['app'],  0, strrpos($menu_mail_menuData['path']['app'],  '/'));
$menu_mail_menuData['path']['link'] = substr($menu_mail_menuData['path']['link'], 0, strrpos($menu_mail_menuData['path']['link'], '/')) . '/';
$menu_mail_menuData['path']['app'] .= '/';
$menu_mail_menuData['url']['app']   = substr($menu_mail_menuData['url']['web'],  0, strrpos($menu_mail_menuData['url']['web'],  '/'));
$menu_mail_menuData['url']['link']  = substr($menu_mail_menuData['url']['app'],  0, strrpos($menu_mail_menuData['url']['app'],  '/'));
if ($_SESSION['scriptcase']['menu_mail']['glo_nm_usa_grupo'] == "S")
{
    $menu_mail_menuData['url']['link']  = substr($menu_mail_menuData['url']['link'], 0, strrpos($menu_mail_menuData['url']['link'], '/'));
}
$menu_mail_menuData['url']['link']  .= '/';
$menu_mail_menuData['url']['app']   .= '/';
...
if (isset($_SESSION['scriptcase']['menu_mail']['session_timeout']['lang'])) {
    $this->str_lang = $_SESSION['scriptcase']['menu_mail']['session_timeout']['lang'];
}
elseif (!isset($_SESSION['scriptcase']['menu_mail']['actual_lang']) || $_SESSION['scriptcase']['menu_mail']['actual_lang'] != $this->str_lang) {
    $_SESSION['scriptcase']['menu_mail']['actual_lang'] = $this->str_lang;
...
include_once("menu_mail_toolbar.php");

$this->tab_grupo[0] = "mail_marketing/";
if ($_SESSION['scriptcase']['menu_mail']['glo_nm_usa_grupo'] != "S")
{
    $this->tab_grupo[0] = "";
}

     $_SESSION['scriptcase']['menu_atual'] = "menu_mail";
     $_SESSION['scriptcase']['menu_apls']['menu_mail'] = array();
     if (isset($_SESSION['scriptcase']['sc_connection']) && !empty($_SESSION['scriptcase']['sc_connection']))
     {
         foreach ($_SESSION['scriptcase']['sc_connection'] as $NM_con_orig => $NM_con_dest)
         {
             if (isset($_SESSION['scriptcase']['menu_mail']['glo_nm_conexao']) && $_SESSION['scriptcase']['menu_mail']['glo_nm_conexao'] == $NM_con_orig)
             {
/*NM*/           $_SESSION['scriptcase']['menu_mail']['glo_nm_conexao'] = $NM_con_dest;
             }
             if (isset($_SESSION['scriptcase']['menu_mail']['glo_nm_perfil']) && $_SESSION['scriptcase']['menu_mail']['glo_nm_perfil'] == $NM_con_orig)
             {
/*NM*/           $_SESSION['scriptcase']['menu_mail']['glo_nm_perfil'] = $NM_con_dest;
             }
             if (isset($_SESSION['scriptcase']['menu_mail']['glo_con_' . $NM_con_orig]))
             {
                 $_SESSION['scriptcase']['menu_mail']['glo_con_' . $NM_con_orig] = $NM_con_dest;
             }
         }
     }
...
if (isset($_SESSION['scriptcase']['menu_mail']['session_timeout']['redir'])) {
...
';
    ...
    if ($_SESSION['scriptcase']['menu_mail']['session_timeout']['redir_tp'] == "R") {
        $SS_cod_html .= "  </HEAD>\r\n";
        $SS_cod_html .= "   <body>\r\n";
    }
    else {
        ...
        $SS_cod_html .= "           <input type=\"button\" name=\"sc_sai_seg\" value=\"OK\" onclick=\"sc_session_redir('" . $_SESSION['scriptcase']['menu_mail']['session_timeout']['redir'] . "');\">\r\n";
        $SS_cod_html .= "     </form>\r\n";
    ...
    if ($_SESSION['scriptcase']['menu_mail']['session_timeout']['redir_tp'] == "R") {
        $SS_cod_html .= "      sc_session_redir('" . $_SESSION['scriptcase']['menu_mail']['session_timeout']['redir'] . "');\r\n";
    }
    ...
    $SS_cod_html .= " </body>\r\n";
    $SS_cod_html .= "</HTML>\r\n";
    unset($_SESSION['scriptcase']['menu_mail']['session_timeout']);
...
if (!isset($_SESSION['sc_session'][1]['menu_mail']['init']))
{
    $_SESSION['sc_session'][1]['menu_mail']['init'] = "";
}
$script_case_init = $_SESSION['sc_session'][1]['menu_mail']['init'];
...
if (isset($_SESSION['sc_session']['SC_parm_violation']) && !isset($_SESSION['scriptcase']['menu_mail']['session_timeout']['redir']))
{
...
{ 
    $nm_url_saida = $_SESSION['scriptcase']['sc_saida_menu_mail'];
}
elseif (!empty($nm_url_saida))
{
    $_SESSION['scriptcase']['sc_url_saida'][$script_case_init]  = $nm_url_saida;
    $_SESSION['scriptcase']['sc_saida_menu_mail'] = $nm_url_saida;
}
else
{
    $_SESSION['scriptcase']['sc_saida_menu_mail'] = (isset($_SERVER['HTTP_REFERER']) && !empty($_SERVER['HTTP_REFERER'])) ? $_SERVER['HTTP_REFERER'] : "javascript:window.close()";
}
$this->sc_Include($path_libs . "/nm_ini_lib.php", "F", "nm_dir_normaliza") ; 
/* Dados do menu em sessao */
$_SESSION['nm_menu'] = array('prod' => $str_root . $_SESSION['scriptcase']['menu_mail']['glo_nm_path_prod'] . '/third/COOLjsMenu/',
                              'url' => $_SESSION['scriptcase']['menu_mail']['glo_nm_path_prod'] . '/third/COOLjsMenu/');

if ((isset($nmgp_outra_jan) && $nmgp_outra_jan == "true") || (isset($_SESSION['scriptcase']['sc_outra_jan']) && $_SESSION['scriptcase']['sc_outra_jan'] == 'menu_mail'))
{
    $_SESSION['sc_session'][1]['menu_mail']['sc_outra_jan'] = true;
     unset($_SESSION['scriptcase']['sc_outra_jan']);
    $_SESSION['scriptcase']['sc_saida_menu_mail'] = "javascript:window.close()";
}
/* Variáveis de Configuração do Menu */
$menu_mail_menuData['iframe'] = TRUE;
...
{
    if (!isset($_SESSION['scriptcase']['sc_apl_seg']['grid_mail_marketing']))
    {
        $_SESSION['scriptcase']['sc_apl_seg']['grid_mail_marketing'] = "on";
    }
}
...
$sc_teste_seg = file($path_apls . $this->tab_grupo[0] . SC_dir_app_name("grid_mail_lista") . "/grid_mail_lista_ini.txt");
if ((!isset($sc_teste_seg[3]) || trim($sc_teste_seg[3]) == "NAO") || (isset($_SESSION['nm_session']['user']['sec']['flag']) && $_SESSION['nm_session']['user']['sec']['flag'] == "N")) 
{
    if (!isset($_SESSION['scriptcase']['sc_apl_seg']['grid_mail_lista']))
    {
        $_SESSION['scriptcase']['sc_apl_seg']['grid_mail_lista'] = "on";
    }
}
if (isset($_SESSION['nm_session']['user']['sec']['flag']) && $_SESSION['nm_session']['user']['sec']['flag'] == "N") 
{ 
...
$sc_teste_seg = file($path_apls . $this->tab_grupo[0] . SC_dir_app_name("grid_mail_meio") . "/grid_mail_meio_ini.txt");
if ((!isset($sc_teste_seg[3]) || trim($sc_teste_seg[3]) == "NAO") || (isset($_SESSION['nm_session']['user']['sec']['flag']) && $_SESSION['nm_session']['user']['sec']['flag'] == "N")) 
{
    if (!isset($_SESSION['scriptcase']['sc_apl_seg']['grid_mail_meio']))
    {
...
if (!isset($_SESSION['scriptcase']['sc_apl_seg']['grid_mail_marketing']) || strtolower($_SESSION['scriptcase']['sc_apl_seg']['grid_mail_marketing']) != "on")
{
    ...
    $str_icon = "scriptcase__NM__ico__NM__date_time_preferences_32.png";
    $icon_aba = "";
    $icon_aba_inactive = "";
...
$str_disabled = "N";
$str_link = "menu_mail_form_php.php?sc_item_menu=item_3&sc_apl_menu=grid_mail_meio&sc_apl_link=" . urlencode($menu_mail_menuData['url']['link']) . "&sc_usa_grupo=" . $_SESSION['scriptcase']['menu_mail']['glo_nm_usa_grupo'] . "";
if (!isset($_SESSION['scriptcase']['sc_apl_seg']['grid_mail_meio']) || strtolower($_SESSION['scriptcase']['sc_apl_seg']['grid_mail_meio']) != "on")
{
    $str_link = "#";
    $str_disabled = "Y";
...
if (isset($_SESSION['scriptcase']['sc_def_menu']['menu_mail']))
{
    $arr_menu_usu = $this->nm_arr_menu_recursiv($_SESSION['scriptcase']['sc_def_menu']['menu_mail']);
    $this->nm_gera_menus($str_menu_usu, $arr_menu_usu, 1, 'menu_mail');
    $menu_mail_menuData['data'] = $str_menu_usu;
}
if (is_file("menu_mail_help.txt"))
{
    $Arq_WebHelp = file("menu_mail_help.txt"); 
        ...
        foreach ($Tmp as $Cada_help)
        {
            ...
            if (!empty($Tmp1[0]) && isset($Tmp1[1]) && !empty($Tmp1[1]) && $Tmp1[0] == "menu" && is_file($str_root . $path_help . $Tmp1[1]))
            {
                ...
                $menu_mail_menuData['data'][] = array(
                    ...
                    'icon_aba' => $icon_aba,
                    'icon_aba_inactive' => $icon_aba_inactive,
                    'target'   => "" . $this->menu_mail_target('_blank') . "",
                    'sc_id'    => "item_Help",
                    'disabled' => $str_disabled,
                    'display'     => "text",
                    'display_position'=> "",
...
if (isset($_SESSION['scriptcase']['sc_menu_del']['menu_mail']) && !empty($_SESSION['scriptcase']['sc_menu_del']['menu_mail']))
{
    $nivel = 0;
    $exclui_menu = false;
    foreach ($menu_mail_menuData['data'] as $i_menu => $cada_menu)
    {
       if (in_array($cada_menu['id'], $_SESSION['scriptcase']['sc_menu_del']['menu_mail']))
       {
          $nivel = $cada_menu['level'];
          $exclui_menu = true;
          unset($menu_mail_menuData['data'][$i_menu]);
       }
    ...
    $menu_mail_menuData['data'] = $Temp_menu;
}

if (isset($_SESSION['scriptcase']['sc_menu_disable']['menu_mail']) && !empty($_SESSION['scriptcase']['sc_menu_disable']['menu_mail']))
{
    $disable_menu = false;
    foreach ($menu_mail_menuData['data'] as $i_menu => $cada_menu)
    {
       if (in_array($cada_menu['id'], $_SESSION['scriptcase']['sc_menu_disable']['menu_mail']))
       {
          $nivel = $cada_menu['level'];
          $disable_menu = true;
          $menu_mail_menuData['data'][$i_menu]['disabled'] = 'Y';
       }
...
/* Cabeçalho HTML */
if ($menu_mail_menuData['iframe'])
{
    $menu_mail_menuData['height'] = '100%';
?>
...
<head>
 ...
 <link rel="stylesheet" href="<?php echo $_SESSION['scriptcase']['menu_mail']['glo_nm_path_prod']; ?>/third/font-awesome/css/all.min.css" type="text/css" media="screen" />
<link rel="stylesheet" type="text/css" href="../_lib/css/_menuTheme/scriptcase_Scriptcase_ScriptCase5_Lilac_hor_<?php echo $_SESSION['scriptcase']['reg_conf']['css_dir']; ?>.css<?php if (@is_file($this->path_css . '_menuTheme/' . "scriptcase_Scriptcase_ScriptCase5_Lilac" . '_' . hor . '.css')) { echo '?scp=' . md5($this->path_css . '_menuTheme/' . "scriptcase_Scriptcase_ScriptCase5_Lilac" . '_' . hor . '.css'); } ?>" />
<style>
   .scTabText {
   }    <?php
...
{
    $menu_mail_menuData['height'] = '30px';
}

/* Arquivos JS */
?>
<script type="text/javascript" src="<?php echo $_SESSION['scriptcase']['menu_mail']['glo_nm_path_prod']; ?>/third/jquery/js/jquery.js"></script>
<script  type="text/javascript" src="<?php echo $_SESSION['scriptcase']['menu_mail']['glo_nm_path_prod']; ?>/third/jquery_plugin/contextmenu/jquery.contextmenu.js"></script>
 <link rel="stylesheet" type="text/css" href="<?php echo $_SESSION['scriptcase']['menu_mail']['glo_nm_path_prod']; ?>/third/jquery_plugin/contextmenu/contextmenu.css" /> 
...
<script type="text/javascript" src="<?php echo $_SESSION['scriptcase']['menu_mail']['glo_nm_path_prod']; ?>/third/sweetalert/sweetalert2.all.min.js"></script>
<script type="text/javascript" src="<?php echo $_SESSION['scriptcase']['menu_mail']['glo_nm_path_prod']; ?>/third/sweetalert/polyfill.min.js"></script>
...
<script type="text/javascript" src="menu_mail_message.js"></script>
...
<?php
 $seq = 0;
 if(isset($menu_mail_menuData['data']) && !empty($menu_mail_menuData['data']))
 {
   foreach ($menu_mail_menuData['data'] as $ind => $dados_menu)
   {
     if ($dados_menu['link'] != "#")
     {
 ...
 if(isset($menu_mail_menuData['data_vertical']) && !empty($menu_mail_menuData['data_vertical']))
 {
   foreach ($menu_mail_menuData['data_vertical'] as $ind => $dados_menu)
   {
     if ($dados_menu['link'] != "#")
     {
         if(empty($dados_menu['hint']))
...
function createIframe(str_id, str_label, str_hint, str_img_on, str_img_off, str_link, tp_apl)
{
    ...
    iframe.name = 'menu_mail_' + str_id + '_iframe';
    iframe.src = str_link;
...
function createAba(str_id)
{
    ...
    var home_style="";
    if(str_id === 'menu_mail'){ home_style=";padding-left:4px;min-height:14px;"; }
...
{
    $('#iframe_menu_mail').hide();
    if (str_id == "")
    {
        $('#iframe_menu_mail').show();
    ...
    for (i = 0; i < Tab_iframes.length; i++) 
    {
        if (Tab_iframes[i] == str_id) 
        {
            ...
            Iframe_atual = "menu_mail_" + Tab_iframes[i] + '_iframe';
...
echo $this->menu_mail_escreveMenu($menu_mail_menuData['data'], $path_imag_cab, $strAlign);
?></tr></table>
</div>
<?php
/* Controle de Iframe */
...
<?php echo $this->nm_show_toolbarmenu('', $saida_apl, $menu_mail_menuData, $path_imag_cab); ?><?php echo $this->nm_gera_degrade(1, $bg_line_degrade, $path_imag_cab); ?>  <tr>
...
<?php
 foreach ($menu_mail_menuData['data'] as $ind => $dados_menu)
 {
     if ($dados_menu['link'] != "#")
     {
         echo "      <iframe id=\"iframe_" . $dados_menu['id'] . "\" name= \"menu_mail_" . $dados_menu['id'] . "_iframe\" frameborder=\"0\" class=\"scMenuIframe\" style=\"display: none;width: 100%; height: 100%;\" src=\"\"></iframe>
";
     }
 }
}
...
/* Controle de Target */
function menu_mail_escreveMenu($arr_menu, $path_imag_cab = '', $strAlign = '')
{
    global $nm_data_fixa;
    $last      = '';
    $itemClass = ' topfirst';
      ...
      <div class='menu__container'>
        ...
        <?php
            ...
            for ($i = 0; $i < sizeof($aMenuItemList); $i++) {
                ...
                ?>
                    <a href="javascript:" onclick="openMenuItem('menu_mail_<?php echo $aMenuItemList[$i]['id']; ?>');" item-href="<?php echo $aMenuItemList[$i]['link']; ?>" id="<?php echo $aMenuItemList[$i]['id']; ?>" title="<?php echo $aMenuItemList[$i]['hint']; ?>" <?php echo $aMenuItemList[$i]['target']; ?> class='menu__link <?php echo $sDisabledClass; ?>'><?php echo $str_item; ?></a>
                <?php
                if ($hasChildrens) {
                ?>
...
function menu_mail_target($str_target)
{
    global $menu_mail_menuData;
    ...
    elseif ($menu_mail_menuData['iframe'])
    {
        return 'menu_mail_iframe';
    }
    else
    {
        return $str_target;
...
function nm_show_toolbarmenu($col_span, $saida_apl, $menu_mail_menuData, $path_imag_cab)
{
}
   ...
   function nm_gera_menus(&$str_line_ret, $arr_menu_usu, $int_level, $nome_aplicacao)
   {
       ...
       foreach ($arr_menu_usu as $arr_item)
       {
           ...
           if ('' != $arr_item['link'])
           {
               ...
               {
                    $str_line['link'] = "menu_mail_form_php.php?sc_item_menu=" . $arr_item['id'] . "&sc_apl_menu=" . $nome_apl . "&sc_apl_link=" . urlencode($menu_mail_menuData['url']['link']) . "&sc_usa_grupo=" . $_SESSION['scriptcase']['menu_mail']['glo_nm_usa_grupo'] . ""; 
               }
               else
               {
                    $str_line['link'] = "menu_mail_form_php.php?sc_item_menu=" . $arr_item['id'] . "&sc_apl_menu=" . $nome_apl . "&sc_apl_link=" . urlencode($menu_mail_menuData['url']['link']) . "&sc_usa_grupo=" . $_SESSION['scriptcase']['menu_mail']['glo_nm_usa_grupo'] . ""; 
           ...
           {
                $str_line['target'] = ('' != $arr_item['target'] && '' != $arr_item['link']) ?  $this->menu_mail_target( $arr_item['target']) : "_self"; 
           }
   ...
   {
        $_SESSION['scriptcase']['menu_mail']['sc_init'][$apl_menu] = rand(2, 10000);
        $_SESSION['sc_session'][$_SESSION['scriptcase']['menu_mail']['sc_init'][$apl_menu]] = array();
        return  $_SESSION['scriptcase']['menu_mail']['sc_init'][$apl_menu];
   }
...
if ($Sem_Session && (!isset($nmgp_start) || $nmgp_start != "SC")) {
    ...
    elseif (is_file($root . $_SESSION['scriptcase']['menu_mail']['glo_nm_path_imag_temp'] . "/sc_apl_default_mail_marketing.txt")) {
        $apl_def = explode(",", file_get_contents($root . $_SESSION['scriptcase']['menu_mail']['glo_nm_path_imag_temp'] . "/sc_apl_default_mail_marketing.txt"));
    }
    if (isset($apl_def)) {
        if ($apl_def[0] != "menu_mail") {
            $_SESSION['scriptcase']['sem_session'] = true;
            if (strtolower(substr($apl_def[0], 0 , 7)) == "http://" || strtolower(substr($apl_def[0], 0 , 8)) == "https://" || substr($apl_def[0], 0 , 2) == "..") {
                $_SESSION['scriptcase']['menu_mail']['session_timeout']['redir'] = $apl_def[0];
            }
            else {
                $_SESSION['scriptcase']['menu_mail']['session_timeout']['redir'] = $path_aplicacao . "/" . SC_dir_app_name($apl_def[0]) . "/index.php";
            }
            $Redir_tp = (isset($apl_def[1])) ? trim(strtoupper($apl_def[1])) : "";
            $_SESSION['scriptcase']['menu_mail']['session_timeout']['redir_tp'] = $Redir_tp;
        }
        if (isset($_COOKIE['sc_actual_lang_mail_marketing'])) {
            $_SESSION['scriptcase']['menu_mail']['session_timeout']['lang'] = $_COOKIE['sc_actual_lang_mail_marketing'];
        }
    }
}
...
$contr_menu_mail = new menu_mail_class;
$contr_menu_mail->menu_mail_menu();
